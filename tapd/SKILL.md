...
```bash
.codex/skills/tapd/config.json
```

如果该文件存在且包含 `access_token`，执行任何 `tapd` 命令前先读取它，并将值注入当前命令环境变量 `TAPD_ACCESS_TOKEN`。不要把该文件内容输出到聊天、日志或提交到代码仓库。
...
```bash
tapd comment add [--author=<可选，默认使用当前登录用户>] [--description=<text>|--file=<path>|stdin] --entry-id=<必需> --entry-type=<stories|bug|bug_remark|tasks> [--reply-id=<可选>]  # 添加评论
tapd comment count [--entry-id] [--entry-type=<stories|bug|bug_remark|tasks>]  # 查询评论数量
tapd comment list [--author] [--entry-id] [--entry-type=<stories|bug|bug_remark|tasks>] [--filter=<可重复，格式：field=OP<value>，支持 LIKE/EQ/CONTAINS 等 OpenAPI 特殊查询语法>] [--limit=10] [--order=<如 created desc>] [--page=1]  # 查询评论列表
